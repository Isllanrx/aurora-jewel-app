// lib/supabase.js
// SUPABASE_API_KEY é a chave anon (pública) — projetada para uso em
// aplicativos client-side e protegida por Row Level Security (RLS).
// NÃO é a service_role key (secreta). Configurar as credenciais abaixo:
export const SUPABASE_URL = "https://bebuzflitiyuevumwwpi.supabase.co";
export const SUPABASE_API_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJlYnV6ZmxpdGl5dWV2dW13d3BpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAxODE2MjksImV4cCI6MjA5NTc1NzYyOX0.Xzm5N5khVnAZ7XfgkhpyvskssmpZMTIyvhmH7E1KGW4";
...
export async function authSignUp(email, password, metadata = {}) {
  ...
  if (!res.ok) throw new Error(data.msg || data.message || data.error_description || data.error || "Erro no cadastro");
  return data;
}
...
export async function authSignIn(email, password) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.msg || data.message || data.error_description || data.error || "E-mail ou senha incorretos");
  return data;
}
