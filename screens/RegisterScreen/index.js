      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        {feedback && (
          <View style={[styles.banner, feedback.type === "success" ? styles.bannerSuccess : styles.bannerError]}>
            <Ionicons name={feedback.type === "success" ? "checkmark-circle" : "alert-circle"} size={18} color={Colors.white} />
            <Text style={styles.bannerText}>{feedback.text}</Text>
          </View>
        )}

        <TouchableOpacity style={[styles.button, loading && styles.buttonDisabled]} onPress={handleSubmit(onSubmit)} disabled={loading} activeOpacity={0.8}>
          {loading ? <ActivityIndicator color={Colors.white} /> : <Text style={styles.buttonText}>{t("register")}</Text>}
        </TouchableOpacity>
