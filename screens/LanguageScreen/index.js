    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {LANGUAGES.map(({ code, label, nameKey, nativeName }) => {
          ...
          return (
            <TouchableOpacity key={code} style={[styles.option, isActive && styles.optionActive]} onPress={() => setSelected(code)} activeOpacity={0.7}>
              <View style={[styles.langCode, isActive && styles.langCodeActive]}>
                <Text style={[styles.langCodeText, isActive && styles.langCodeTextActive]}>{label}</Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[styles.langName, isActive && styles.langNameActive]}>{t(nameKey)}</Text>
        ...
        {showSuccess && <Text style={styles.successMsg}>{t("languageSaved")}</Text>}
      </ScrollView>
    </SafeAreaView>
  );
}
